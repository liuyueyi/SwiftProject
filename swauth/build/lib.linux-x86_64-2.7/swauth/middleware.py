...
from swauth.tree import generateTree, match
...
class Swauth(object):
    ...
    def get_groups(self, env, token):
		...
		n = token.index('attr')
                path = quote('/v1/%s/.token_%s/%s' %
                             (self.auth_account, token[n-1], token))
                resp = self.make_pre_authed_request(
                    env, 'GET', path).get_response(self.app)
                if resp.status_int // 100 != 2:
                    return None
	...
	logging.info(req.path)
		...
		attr_policy = getattr(req, 'attr_policy', None)
		if attr_policy :
		    candidate_token = req.headers.get('x-auth-token')
		    n=candidate_token.index("attr")
	    	    candidate_attr_token=unquote(candidate_token[n+4:])
		    user_attr = []
		    for value in candidate_attr_token.split(' '):
			user_attr.append(value)
		    policy_tree=generateTree(attr_policy)
		    if match(policy_tree, user_attr) > 0:
			#req.environ['swift_owner'] = True
		    	return None
		    else:
			return self.denied_response(req)
    ...
    def handle_get_account(self, req):
	    ...
	    account_attr = resp.headers['X-Container-Meta-Account-Attr']
	    if not account_attr:
		account_attr = 'None'
	    account_inst = resp.headers['X-Container-Meta-Account-Inst']
	    if not account_inst:
		account_inst = 'None'
            sublisting = json.loads(resp.body)
            if not sublisting:
        ...
        return Response(body=json.dumps({'account_id': account_id,
                                    'services': services, 'users': listing, 'account_attr': account_attr, 'account_inst': account_inst}))
    ...
    def handle_put_account(self, req):
        ...
        """
	account_attr = req.headers.get('X-Group-Attr')
	account_inst = req.headers.get('X-Group-Inst')
        if not self.is_reseller_admin(req):
            return self.denied_response(req)
        account = req.path_info_pop()
        if req.path_info or not account or account[0] == '.':
        ...
        resp = self.make_pre_authed_request(
            ...
            headers={'X-Container-Meta-Account-Id': '%s%s' % (
                self.reseller_prefix, account_suffix), 'X-Container-Meta-Account-Attr': account_attr, 'X-Container-Meta-Account-Inst': account_inst}).get_response(self.app)
        if resp.status_int // 100 != 2:
	...
	attr=req.headers.get('x-auth-user-attr')
	group_user=req.headers.get('x-user-group')
        if reseller_admin:
            admin = True
        if req.path_info or not account or account[0] == '.' or not user or \
                user[0] == '.' or not key:
    ...
    def handle_delete_user(self, req):
	    ...
	    n = candidate_token.index('attr')
            path = quote('/v1/%s/.token_%s/%s' %
                (self.auth_account, candidate_token[n-1], candidate_token))
            resp = self.make_pre_authed_request(
                req.environ, 'DELETE', path).get_response(self.app)
            if resp.status_int // 100 != 2 and resp.status_int != 404:
	...
	user_attr=user_detail.get('attr')
	group_user=user_detail.get('group_user')
	...
	attr_token = None
	    ...
	    attr_token = 'attr%s' % quote(user_attr)
            # Save token info
	    token = token + attr_token
	    n = token.index('attr')
            path = quote('/v1/%s/.token_%s/%s' %
                         (self.auth_account, token[n-1], token))
            try:
