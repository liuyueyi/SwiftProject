    ...
    try:
        ...
        while True:
            ...
            for item in items:
                ...
                if not options.long and not options.human:  # just disploy the name and subdir if there is 
                    thread_manager.print_msg(
                        item.get('name', item.get('subdir')))
                    # add by wuzebang 2013/12/27
                    result['subdir'] = item.get('subdir') 
                    # end
                else:
                    ...
                    if len(args) == 0:    # listing containers
                        ...
                        if not options.totals:
                            thread_manager.print_msg("%s %10s %8s %s",
                                                     ...
                                                     item_name)
                            # add by wuzebang 2013/12/27
                            result['byte_str'] = byte_str  # content size
                            result['date'] = date       #last modified date
                            result['xtime'] = xtime   # last_modified time
                            # end
...
def st_stat(parser, args, thread_manager):
    ...
    args = args[1:]
    conn = get_conn(options)
    if not args: # get the information for the account
        try:
        	# wuzebang
        	# tt = command_helpers.stat_account(conn, options, thread_manager)        	
            return command_helpers.stat_account(conn, options, thread_manager)
        except ClientException as err:
            if err.http_status != 404:
                raise
            thread_manager.error('Account not found')
    elif len(args) == 1:    # get the information for the container
        if '/' in args[0]:
            print >> stderr, 'WARNING: / in container name; you might have ' \
                             'meant %r instead of %r.' % \
                             (args[0].replace('/', ' ', 1), args[0])
        try:
            return command_helpers.stat_container(conn, options, args,
                                           thread_manager)
        except ClientException as err:
            if err.http_status != 404:
                raise
            thread_manager.error('Container %r not found', args[0])
    elif len(args) == 2:    # get the informatioo for the object
        try:
            return command_helpers.stat_object(conn, options, args, thread_manager)
        except ClientException as err:
            if err.http_status != 404:
                raise
...
def st_post(parser, args, thread_manager):
    ...
    conn = get_conn(options)
    if not args: # post account meta infomation
        ...
        try:
            ...
            if err.http_status != 404:
                raise
            thread_manager.error('Account not found')
    elif len(args) == 1: # post container meta information
        if '/' in args[0]:
            print >> stderr, 'WARNING: / in container name; you might have ' \
                             'meant %r instead of %r.' % \
                             (args[0].replace('/', ' ', 1), args[0])
    ...
    elif len(args) == 2: #post object meta information
...
'''.strip('\n') % globals())
    ...
    (options, args) = parse_args(parser, content, enforce_requires=False)
    ...
    with MultiThreadingManager() as thread_manager:
        ...
        try:
            return globals()['st_%s' % args[0]](parser, content, thread_manager)
...
for item in os.listdir(path):
	...
	command.extend(['-U', 'Iris:Iris', '-K', 'Iris', 'upload', 'BT', itemsrc])
	print 'command is:', command
	res = opt(command)
#		print 'The receive is : \n', repr(opt(command))
